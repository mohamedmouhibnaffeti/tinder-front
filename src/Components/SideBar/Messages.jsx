...
const Messages = ({member}) => {
    ...
    return(
        <div className="flex flex-col w-full" onClick={()=>{setOpen(true); setChatID(chatID);setChatMember(member)}}>
            <div className={`rounded-lg border border-gray-800 w-full h-24 flex items-center bg-zinc-800 hover:bg-gray-800 hover:border-y hover:border-y-gray-400 hover:border-r-4 hover:border-r-red-700 px-2 gap-5 cursor-pointer text-gray-300 hover:text-pink-700 ${ChatID === chatID ? "border-r-2 border-r-red-800" : ""} `}>
                <div>
